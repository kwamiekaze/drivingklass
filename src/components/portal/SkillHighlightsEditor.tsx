...
import { Plus, X, RotateCcw, Star, TrendingUp, AlertTriangle, Info, Pencil } from "lucide-react";
...
export interface SkillHighlightItem {
  ...
  /** Custom (free-text) entry — excluded from charts/graphs. */
  custom?: boolean;
}
...
function computeAutoSuggestions(
  ...
  const skillScores = SKILL_KEYS.map(key => ({
    key, label: SKILL_LABELS[key] || key, score: currentRatings[key] || 0,
  }));
  const sorted = [...skillScores].sort((a, b) => b.score - a.score);
  const strongest = sorted.slice(0, 2).filter(s => s.score > 0).map(s => ({
    skill_key: s.key, skill_label: s.label, source_type: "auto" as const,
  }));
  const weakest = [...skillScores].filter(s => s.score > 0).sort((a, b) => a.score - b.score);
  const focusAreas = weakest.slice(0, 2).map(s => ({
    skill_key: s.key, skill_label: s.label, source_type: "auto" as const,
  }));
  ...
  if (priorReports && priorReports.length > 0) {
    ...
    mostImproved = gains.slice(0, 2).map(g => ({
      skill_key: g.key, skill_label: g.label, source_type: "auto" as const,
    }));
  }
  if (mostImproved.length === 0 && strongest.length > 0) mostImproved = [strongest[0]];
...
export function SkillHighlightsEditor({ strongest, mostImproved, focusAreas, onChange, currentRatings, priorReports, isFirstLesson }: Props) {
  ...
  const handleAddCustom = (field: "strongest" | "mostImproved" | "focusAreas", items: SkillHighlightItem[]) => {
    const text = (customDrafts[field] || "").trim();
    if (!text || items.length >= MAX_ITEMS) return;
    const key = `custom:${field}:${Date.now()}`;
    onChange(field, [...items, { skill_key: key, skill_label: text, source_type: "manual", custom: true }]);
    setCustomDrafts(d => ({ ...d, [field]: "" }));
  };
  ...
  const renderGroup = (
    ...
    items: SkillHighlightItem[], colorClass: string,
  ) => {
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline" className="text-[10px] px-1.5 py-0">{items.length}/{MAX_ITEMS}</Badge>
          </div>
          <Button type="button" variant="ghost" size="sm" className="h-7 text-xs text-muted-foreground hover:text-foreground" onClick={() => handleReset(field)}>
        ...
        <div className="space-y-1.5">
          {items.map((item) => (
            <div key={item.skill_key} className={`flex items-center justify-between rounded-md border border-border/60 bg-card/60 px-3 py-1.5 ${colorClass}`}>
              <div className="flex items-center gap-2">
                ...
                {item.custom && (
                  <Badge variant="outline" className="text-[9px] px-1 py-0 text-muted-foreground border-muted-foreground/30">
                    custom · not on charts
                  </Badge>
                )}
                {!item.custom && item.source_type === "auto" && (
                  <Badge variant="outline" className="text-[9px] px-1 py-0 text-muted-foreground border-muted-foreground/30">auto</Badge>
                )}
              </div>
              <Button type="button" variant="ghost" size="sm" className="h-6 w-6 p-0 text-muted-foreground hover:text-destructive" onClick={() => handleRemove(field, items, item.skill_key)}>
        ...
        {items.length < MAX_ITEMS && (
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Input
                value={customDrafts[field]}
                onChange={(e) => setCustomDrafts(d => ({ ...d, [field]: e.target.value }))}
                placeholder={`Or write a custom ${label.toLowerCase()} note...`}
                className="h-9 text-sm"
                onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddCustom(field, items); } }}
              />
              <Button type="button" variant="outline" size="sm" className="h-9 gap-1" onClick={() => handleAddCustom(field, items)} disabled={!customDrafts[field]?.trim()}>
                <Pencil className="h-3.5 w-3.5" />
                Add
              </Button>
            </div>
            <p className="text-[10px] text-muted-foreground">Custom entries appear on the report but are excluded from the skill chart.</p>
          </div>
        )}
      </div>
    );
  ...
  return (
    <Card className="luxury-card">
      <CardHeader>
        ...
        <p className="text-xs text-muted-foreground">Auto-suggested from ratings. Pick from the list or write your own.</p>
      </CardHeader>
      <CardContent className="space-y-5">
        {renderGroup("strongest", "Strongest",
          <Star className="h-4 w-4 text-green-600 dark:text-green-500" />, strongest, "border-l-2 border-l-green-500/40")}
        <div className="space-y-2">
          <label className="flex items-start gap-2 rounded-md border border-border/60 bg-muted/30 px-3 py-2 cursor-pointer">
            <Checkbox checked={excludeMostImproved} onCheckedChange={(v) => setExcludeMostImproved(!!v)} className="mt-0.5" />
          ...
          {!excludeMostImproved && renderGroup("mostImproved", "Most Improved",
            <TrendingUp className="h-4 w-4 text-blue-600 dark:text-blue-400" />, mostImproved, "border-l-2 border-l-blue-500/40")}
        </div>
        {renderGroup("focusAreas", "Focus Areas",
          <AlertTriangle className="h-4 w-4 text-orange-600 dark:text-orange-400" />, focusAreas, "border-l-2 border-l-orange-500/40")}
      </CardContent>
    </Card>
  );
}
