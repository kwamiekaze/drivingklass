// Shared recipient-name resolution for every email path.
// Always produces a friendly FIRST name, never a generic role word.
...
export function firstNameFrom(value?: string | null): string {
  ...
  for (const part of parts) {
    ...
    // Skip standalone initials like "J." when a longer name follows.
...
export function profileFirstName(
  profile?: { first_name?: string | null; full_name?: string | null; email?: string | null } | null,
): string {
  if (!profile) return ''
...
/** Greeting-safe name: returns the resolved first name or '' so templates fall back to "there". */
